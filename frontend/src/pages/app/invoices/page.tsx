...
import { Invoice, InvoiceQuery } from "@/types/invoice";
...
export default function InvoicesIndex() {
  ...
  const [orderBy, setOrderBy] = useState<keyof Invoice>("createdAt" as any);
  ...
  const headCells: HeadCell[] = [
    { id: "invoiceNumber", label: t("Invoice Number"), numeric: false },
    { id: "student", label: t("Student"), numeric: false },
    { id: "amount", label: t("Amount"), numeric: true },
    { id: "status", label: t("Status"), numeric: false },
    { id: "dueDate", label: t("Due Date"), numeric: false },
    { id: "paidDate", label: t("Paid Date"), numeric: false },
  ...
  const handleSelectAllClick = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = invoices.map((n) => n._id as string);
  ...
  const handleExport = () => {
    ...
    InvoiceService.exportInvoices(query);
  };
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        await InvoiceService.importInvoices(file);
      ...
      <Box className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <Box>
          ...
          <Breadcrumbs>
            ...
            <Typography color="text.primary">{t("Invoices")}</Typography>
          </Breadcrumbs>
        </Box>
        ...
        <Box className="flex items-center gap-2 flex-wrap">
          ...
          <Button variant="surface" color="primary" startIcon={<NiPlus size="medium" />} onClick={() => navigate(`/${role}/invoices/create`)}>
            {t("New Invoice")}
          </Button>
        </Box>
      </Box>
      ...
      <Box className="bg-background-paper mb-4 p-4 rounded-xl shadow-sm">
        <Grid container spacing={2} alignItems="center">
          <Grid size={{ xs: 12, md: 3 }}>
            <TextField fullWidth size="small" placeholder={t("Search by invoice number, student...")}
        ...
        <Table className="mt-4 w-full min-w-3xl">
          <TableHead className="border-b-1 border-gray-200">
            <TableRow>
              <TableCell sx={{ ...headCellSx, width: 48, pr: 0 }}>
                <Checkbox color="primary" indeterminate={selected.length > 0 && selected.length < invoices.length}
                  checked={invoices.length > 0 && selected.length === invoices.length} onChange={handleSelectAllClick}
                ...
                <TableCell key={headCell.id} align={headCell.numeric ? "right" : "left"}
                  ...
                  {headCell.id !== "actions" ? (
                    <TableSortLabel active={orderBy === headCell.id} direction={orderBy === headCell.id ? order : "asc"}
                      onClick={() => handleRequestSort(headCell.id as keyof Invoice)} IconComponent={NiArrowUp}>
                      {headCell.label}
                    </TableSortLabel>
                  ) : headCell.label}
                </TableCell>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={8} align="center" sx={{ ...cellSx, py: 8 }}><Typography variant="body1">{t("No invoices found")}</Typography></TableCell></TableRow>
            ) : (
              invoices.map((invoice, index) => {
                const id = invoice._id as string;
                const isItemSelected = isSelected(id);
                const labelId = `invoice-checkbox-${index}`;
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected} tabIndex={-1} key={id} selected={isItemSelected}
                    ...
                    <TableCell component="th" id={labelId} scope="row" sx={{ ...cellSx, fontWeight: 500 }} onClick={() => navigate(`/${role}/invoices/view/${id}`)}>{invoice.invoiceNumber}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/invoices/view/${id}`)}>{getStudentName(invoice.student)}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/invoices/view/${id}`)} align="right">{formatCurrency(invoice.amount)}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/invoices/view/${id}`)}>
                      <Chip label={t(invoice.status || "")} size="small" color={getStatusColor(invoice.status)} />
                    </TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/invoices/view/${id}`)}>{formatDate(invoice.dueDate)}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/invoices/view/${id}`)}>{formatDate(invoice.paidDate)}</TableCell>
                    <TableCell align="right" sx={cellSx}>
                      <Box className="flex justify-end gap-1">
                        <Tooltip title={t("View")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/invoices/view/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiEyeOpen size="medium" />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={t("Edit")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/invoices/edit/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiPen size="medium" />
                          </IconButton>
                        </Tooltip>
