...
export default function EventsIndex() {
  ...
  const [orderBy, setOrderBy] = useState<keyof Event>("createdAt" as any);
  ...
  const headCells: HeadCell[] = [
    ...
    { id: "endDate", label: t("End Date"), numeric: false },
    { id: "location", label: t("Location"), numeric: false },
  ...
  const handleSelectAllClick = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = events.map((n) => n._id as string);
  ...
  const handleExport = () => {
    ...
    EventService.exportEvents(query);
  };
      ...
      <Box className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <Box>
          ...
          <Breadcrumbs>
            ...
            <Typography color="text.primary">{t("Events")}</Typography>
          </Breadcrumbs>
        </Box>
        ...
        <Box className="flex items-center gap-2 flex-wrap">
          ...
          <Button variant="surface" color="primary" startIcon={<NiPlus size="medium" />} onClick={() => navigate(`/${role}/events/create`)}>
            {t("New Event")}
          </Button>
        </Box>
      </Box>
        ...
        <Table className="mt-4 w-full min-w-3xl">
          <TableHead className="border-b-1 border-gray-200">
            <TableRow>
              <TableCell sx={{ ...headCellSx, width: 48, pr: 0 }}>
                <Checkbox color="primary" indeterminate={selected.length > 0 && selected.length < events.length}
                  checked={events.length > 0 && selected.length === events.length} onChange={handleSelectAllClick}
                ...
                <TableCell key={headCell.id} align={headCell.numeric ? "right" : "left"}
                  ...
                  {headCell.id !== "actions" ? (
                    <TableSortLabel active={orderBy === headCell.id} direction={orderBy === headCell.id ? order : "asc"}
                      onClick={() => handleRequestSort(headCell.id as keyof Event)} IconComponent={NiArrowUp}>
                      {headCell.label}
                    </TableSortLabel>
                  ) : headCell.label}
                </TableCell>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={7} align="center" sx={{ ...cellSx, py: 8 }}><Typography variant="body1">{t("No events found")}</Typography></TableCell></TableRow>
            ) : (
              events.map((event, index) => {
                const id = event._id as string;
                const isItemSelected = isSelected(id);
                const labelId = `event-checkbox-${index}`;
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected} tabIndex={-1} key={id} selected={isItemSelected}
                    ...
                    <TableCell component="th" id={labelId} scope="row" sx={{ ...cellSx, fontWeight: 500 }} onClick={() => navigate(`/${role}/events/view/${id}`)}>{event.title}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/events/view/${id}`)}>
                      <Box>{formatDate(event.startDate)}</Box>
                      <Typography variant="caption" color="text.secondary">{formatTime(event.startDate)}</Typography>
                    </TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/events/view/${id}`)}>
                      <Box>{formatDate(event.endDate)}</Box>
                      <Typography variant="caption" color="text.secondary">{event.endDate ? formatTime(event.endDate) : "-"}</Typography>
                    </TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/events/view/${id}`)}>{event.location || "-"}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/events/view/${id}`)}>
                      <Chip label={t(event.status || "scheduled")} size="small" color={getStatusColor(event.status)} />
                    </TableCell>
                    <TableCell align="right" sx={cellSx}>
                      <Box className="flex justify-end gap-1">
                        <Tooltip title={t("View")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/events/view/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiEyeOpen size="medium" />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={t("Edit")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/events/edit/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiPen size="medium" />
                          </IconButton>
                        </Tooltip>
