...
export default function TransactionsIndex() {
  ...
  const [orderBy, setOrderBy] = useState<keyof Transaction>("date" as any);
  ...
  }, [page, rowsPerPage, search, typeFilter, startDate, endDate]);
  ...
  const handleSelectAllClick = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = transactions.map((n) => n._id as string);
  ...
  const handleExport = () => {
    ...
    TransactionService.exportTransactions(query);
  };
    ...
    <Box className="p-4">
      <Box className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Box className="flex items-center gap-2 flex-wrap">
          ...
          <Button variant="surface" color="primary" startIcon={<NiPlus size="medium" />} onClick={() => navigate(`/${role}/transactions/create`)}>
            {t("New Transaction")}
          </Button>
        </Box>
      </Box>
      ...
      <Box className="bg-background-paper mb-4 p-4 rounded-xl shadow-sm">
        <Grid container spacing={2} alignItems="center">
          <Grid size={{ xs: 12, md: 3 }}>
            <TextField fullWidth size="small" placeholder={t("Search by account, description...")}
          ...
          <Grid size={{ xs: 12, md: 3 }}>
            <TextField fullWidth select size="small" placeholder={t("Type")}
              value={typeFilter} onChange={(e) => setTypeFilter(e.target.value as TransactionType | "")}>
        ...
        <Table className="mt-4 w-full min-w-3xl">
          <TableHead className="border-b-1 border-gray-200">
            <TableRow>
              <TableCell sx={{ ...headCellSx, width: 48, pr: 0 }}>
                <Checkbox color="primary" indeterminate={selected.length > 0 && selected.length < transactions.length}
                  checked={transactions.length > 0 && selected.length === transactions.length} onChange={handleSelectAllClick}
                ...
                <TableCell key={headCell.id} align={headCell.numeric ? "right" : "left"}
                  ...
                  {headCell.id !== "actions" ? (
                    <TableSortLabel active={orderBy === headCell.id} direction={orderBy === headCell.id ? order : "asc"}
                      onClick={() => handleRequestSort(headCell.id as keyof Transaction)} IconComponent={NiArrowUp}>
                      {headCell.label}
                    </TableSortLabel>
                  ) : headCell.label}
                </TableCell>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={8} align="center" sx={{ ...cellSx, py: 8 }}><Typography variant="body1">{t("No transactions found")}</Typography></TableCell></TableRow>
            ) : (
              transactions.map((transaction, index) => {
                const id = transaction._id as string;
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected} tabIndex={-1} key={id} selected={isItemSelected}
                    ...
                    <TableCell component="th" id={labelId} scope="row" sx={{ ...cellSx, fontWeight: 500 }} onClick={() => navigate(`/${role}/transactions/view/${id}`)}>{formatDate(transaction.date)}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/transactions/view/${id}`)}>{transaction.account || "-"}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/transactions/view/${id}`)}>
                      <Chip label={t(transaction.type || "")} size="small" color={getTypeColor(transaction.type)} />
                    </TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/transactions/view/${id}`)} align="right" sx={{ color: transaction.type === "credit" ? "success.main" : "error.main", fontWeight: 600 }}>
                      {transaction.type === "credit" ? "+" : "-"}{formatCurrency(transaction.amount)}
                    </TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/transactions/view/${id}`)}>{transaction.description || "-"}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/transactions/view/${id}`)}>{transaction.reference || "-"}</TableCell>
                    <TableCell align="right" sx={cellSx}>
                      <Box className="flex justify-end gap-1">
                        <Tooltip title={t("View")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/transactions/view/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiEyeOpen size="medium" />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={t("Edit")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/transactions/edit/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiPen size="medium" />
                          </IconButton>
                        </Tooltip>
