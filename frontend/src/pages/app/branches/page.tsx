...
export default function BranchesIndex() {
  ...
  const [orderBy, setOrderBy] = useState<keyof Branch>("createdAt" as any);

  const [selected, setSelected] = useState<readonly string[]>([]);
  const [search, setSearch] = useState("");

  const headCells: HeadCell[] = [
    { id: "name", label: t("Branch Name"), numeric: false },
    { id: "code", label: t("Code"), numeric: false },
    { id: "address", label: t("Address"), numeric: false },
    { id: "phone", label: t("Phone"), numeric: false },
    { id: "email", label: t("Email"), numeric: false },
    { id: "manager", label: t("Manager"), numeric: false },
  ...
  const fetchBranches = useCallback(async () => {
    ...
    try {
      ...
      const response = await BranchService.getBranches(query);
      if (response.success) {
  ...
  const handleRequestSort = (property: keyof Branch) => {
  ...
  const handleSelectAllClick = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = branches.map((n) => n._id as string);
      ...
      <Box className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <Box>
          ...
          <Breadcrumbs>
            ...
            <Typography color="text.primary">{t("Branches")}</Typography>
          </Breadcrumbs>
        </Box>
        ...
        <Box className="flex items-center gap-2 flex-wrap">
          <Button
            ...
            onClick={() => navigate(`/${role}/branches/create`)}
          >
            {t("New Branch")}
          </Button>
        </Box>
        ...
        <Grid container spacing={2} alignItems="center">
          <Grid size={{ xs: 12, md: 4 }}>
            <TextField
              fullWidth
              size="small"
              placeholder={t("Search by name, code, manager...")}
              value={search}
        ...
        <Table className="mt-4 w-full min-w-3xl">
                ...
                >
                  {headCell.id !== "actions" ? (
                    <TableSortLabel
                      ...
                      onClick={() => handleRequestSort(headCell.id as keyof Branch)}
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={8} align="center" sx={{ ...cellSx, py: 8 }}>
                  <Typography variant="body1">{t("No branches found")}</Typography>
                </TableCell>
              </TableRow>
            ) : (
              branches.map((branch, index) => {
                const id = branch._id as string;
                const isItemSelected = isSelected(id);
                const labelId = `branch-checkbox-${index}`;
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell component="th" id={labelId} scope="row" sx={{ ...cellSx, fontWeight: 500 }} onClick={() => navigate(`/${role}/branches/view/${id}`)}>
                      {branch.name}
                    </TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/branches/view/${id}`)}>{branch.code}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/branches/view/${id}`)}>{branch.address}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/branches/view/${id}`)}>{branch.phone}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/branches/view/${id}`)}>{branch.email}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/branches/view/${id}`)}>{branch.manager}</TableCell>
                    <TableCell align="right" sx={cellSx}>
                      <Box className="flex justify-end gap-1">
                        <Tooltip title={t("View")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/branches/view/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiEyeOpen size="medium" />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={t("Edit")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/branches/edit/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiPen size="medium" />
                          </IconButton>
                        </Tooltip>
