...
export default function NotificationsIndex() {
  ...
  const [orderBy, setOrderBy] = useState<keyof Notification>("createdAt" as any);
  ...
  const headCells: HeadCell[] = [
    ...
    { id: "message", label: t("Message"), numeric: false },
    { id: "type", label: t("Type"), numeric: false },
    { id: "isRead", label: t("Status"), numeric: false },
    { id: "createdAt", label: t("Date"), numeric: false },
  ...
  }, [page, rowsPerPage, search, isRead]);

  useEffect(() => {
    fetchNotifications();
  }, [fetchNotifications]);

  const handleRequestSort = (property: keyof Notification) => {
  ...
  const handleSelectAllClick = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = notifications.map((n) => n._id as string);
    ...
    <Box className="p-4">
      <Box className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <Box>
          ...
          <Breadcrumbs>
            ...
            <Typography color="text.primary">{t("Notifications")}</Typography>
          </Breadcrumbs>
        </Box>

        <Box className="flex items-center gap-2 flex-wrap">
          <Button variant="surface" color="primary" startIcon={<NiCheck size="medium" />} onClick={handleMarkAllAsRead}>
            {t("Mark All Read")}
          </Button>
        </Box>
      </Box>
      ...
      <Box className="bg-background-paper mb-4 p-4 rounded-xl shadow-sm">
        <Grid container spacing={2} alignItems="center">
          <Grid size={{ xs: 12, md: 4 }}>
            <TextField fullWidth size="small" placeholder={t("Search by title...")}
          ...
          <Grid size={{ xs: 12, md: 3 }}>
            <TextField fullWidth select size="small" placeholder={t("Status")}
              value={isRead} onChange={(e) => setIsRead(e.target.value)}>
              <MenuItem value="">{t("All")}</MenuItem>
              <MenuItem value="true">{t("Read")}</MenuItem>
              <MenuItem value="false">{t("Unread")}</MenuItem>
            </TextField>
          </Grid>
        </Grid>
      </Box>
      ...
      <TableContainer className="bg-background-paper rounded-xl shadow-sm" sx={{ p: 1 }}>
        <Table className="mt-4 w-full min-w-3xl">
          <TableHead className="border-b-1 border-gray-200">
            <TableRow>
              <TableCell sx={{ ...headCellSx, width: 48, pr: 0 }}>
                <Checkbox color="primary" indeterminate={selected.length > 0 && selected.length < notifications.length}
                  checked={notifications.length > 0 && selected.length === notifications.length} onChange={handleSelectAllClick}
                  icon={<CheckboxSmallEmptyOutlined />} checkedIcon={<CheckboxSmallChecked />} />
              </TableCell>
                ...
                <TableCell key={headCell.id} align={headCell.numeric ? "right" : "left"}
                  ...
                  {headCell.id !== "actions" ? (
                    <TableSortLabel active={orderBy === headCell.id} direction={orderBy === headCell.id ? order : "asc"}
                      onClick={() => handleRequestSort(headCell.id as keyof Notification)} IconComponent={NiArrowUp}>
                      {headCell.label}
                    </TableSortLabel>
                  ) : headCell.label}
                </TableCell>
          ...
          <TableBody>
            {loading ? (
              <TableRow><TableCell colSpan={7} align="center" sx={{ ...cellSx, py: 8 }}><Typography variant="body1">{t("Loading...")}</Typography></TableCell></TableRow>
            ) : notifications.length === 0 ? (
              <TableRow><TableCell colSpan={7} align="center" sx={{ ...cellSx, py: 8 }}><Typography variant="body1">{t("No notifications found")}</Typography></TableCell></TableRow>
            ) : (
              notifications.map((notification, index) => {
                const id = notification._id as string;
                const isItemSelected = isSelected(id);
                const labelId = `notification-checkbox-${index}`;

                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected} tabIndex={-1} key={id} selected={isItemSelected}
                    sx={{ cursor: "pointer", borderRadius: 2, transition: "background-color 0.15s ease", "&:hover": { bgcolor: "action.hover" }, "&.Mui-selected": { bgcolor: "primary.50" }, ...(!notification.isRead && { bgcolor: "primary.50" }) }}>
                    <TableCell sx={{ ...cellSx, width: 48, pr: 0 }} onClick={() => handleClick(id)}>
                      <Checkbox color="primary" checked={isItemSelected} icon={<CheckboxSmallEmptyOutlined />} checkedIcon={<CheckboxSmallChecked />} />
                    </TableCell>
                    <TableCell component="th" id={labelId} scope="row" sx={{ ...cellSx, fontWeight: notification.isRead ? 400 : 600 }}>
                      {notification.title}
                    </TableCell>
                    <TableCell sx={cellSx}>{notification.message}</TableCell>
                    <TableCell sx={cellSx}>
                      <Chip label={t(notification.type || "info")} size="small" color={getTypeColor(notification.type)} />
                    </TableCell>
                    <TableCell sx={cellSx}>
                      <Chip label={notification.isRead ? t("Read") : t("Unread")} size="small" color={notification.isRead ? "success" : "warning"} />
                    </TableCell>
                    <TableCell sx={cellSx}>{formatDate(notification.createdAt)}</TableCell>
                    <TableCell align="right" sx={cellSx}>
                      <Box className="flex justify-end gap-1">
                        {!notification.isRead && (
                          <Tooltip title={t("Mark as Read")}>
                            <IconButton size="small" onClick={() => handleMarkAsRead(id)}
                        ...
                        {notification.actionUrl && (
                          <Tooltip title={t("View")}>
                            <IconButton size="small" component="a" href={notification.actionUrl}
                              sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                              <NiBell size="medium" />
                            </IconButton>
                          </Tooltip>
