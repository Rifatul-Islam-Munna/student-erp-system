...
export default function TeachersIndex() {
  ...
  const [orderBy, setOrderBy] = useState<keyof Teacher>("createdAt" as any);
  ...
  const headCells: HeadCell[] = [
    ...
    { id: "specialization", label: t("Specialization"), numeric: false },
  ...
  const fetchTeachers = useCallback(async () => {
    ...
    try {
      ...
      const response = await TeacherService.getTeachers(query);
      if (response.success) {
  ...
  const handleRequestSort = (property: keyof Teacher) => {
  ...
  const handleSelectAllClick = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = teachers.map((n) => n._id as string);
  ...
  const handleExport = () => {
    ...
    TeacherService.exportTeachers(query);
  };
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        await TeacherService.importTeachers(file);
      ...
      <Box className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <Box>
          ...
          <Breadcrumbs>
            ...
            <Typography color="text.primary">{t("Teachers")}</Typography>
          </Breadcrumbs>
        </Box>
        ...
        <Box className="flex items-center gap-2 flex-wrap">
          ...
          <Button
            ...
            onClick={() => navigate(`/${role}/teachers/create`)}
          >
            {t("New Teacher")}
          </Button>
        </Box>
        ...
        <Table className="mt-4 w-full min-w-3xl">
                ...
                >
                  {headCell.id !== "actions" ? (
                    <TableSortLabel
                      ...
                      onClick={() => handleRequestSort(headCell.id as keyof Teacher)}
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={7} align="center" sx={{ ...cellSx, py: 8 }}>
                  <Typography variant="body1">{t("No teachers found")}</Typography>
                </TableCell>
              </TableRow>
            ) : (
              teachers.map((teacher, index) => {
                ...
                const labelId = `teacher-checkbox-${index}`;
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell component="th" id={labelId} scope="row" sx={{ ...cellSx, fontWeight: 500 }} onClick={() => navigate(`/${role}/teachers/view/${id}`)}>
                      {teacher.fullName}
                    </TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/teachers/view/${id}`)}>{teacher.email}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/teachers/view/${id}`)}>{teacher.phone}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/teachers/view/${id}`)}>{teacher.specialization}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/teachers/view/${id}`)}>{getStatusLabel(teacher.status || "")}</TableCell>
                    <TableCell align="right" sx={cellSx}>
                      <Box className="flex justify-end gap-1">
                        <Tooltip title={t("View")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/teachers/view/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiEyeOpen size="medium" />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={t("Edit")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/teachers/edit/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiPen size="medium" />
                          </IconButton>
                        </Tooltip>
