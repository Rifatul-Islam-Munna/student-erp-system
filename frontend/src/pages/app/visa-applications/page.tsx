...
interface HeadCell {
  id: keyof VisaApplication | "actions";
  label: string;
  numeric: boolean;
}
...
export default function VisaApplicationsIndex() {
  ...
  const [orderBy, setOrderBy] = useState<keyof VisaApplication>("createdAt" as any);
  ...
  const headCells: HeadCell[] = [
    { id: "passportNumber", label: t("Passport Number"), numeric: false },
    { id: "visaType", label: t("Visa Type"), numeric: false },
    { id: "applicationDate", label: t("Application Date"), numeric: false },
    { id: "interviewDate", label: t("Interview Date"), numeric: false },
  ...
  const fetchVisaApplications = useCallback(async () => {
    setLoading(true);
    try {
      const query: VisaApplicationQuery = {
        page: page + 1,
        limit: rowsPerPage,
        search: search,
        status: statusFilter,
      ...
      const response = await VisaApplicationService.getVisaApplications(query);
      if (response.success) {
        setVisaApplications(Array.isArray(response.data) ? response.data : []);
  ...
  }, [page, rowsPerPage, search, statusFilter, startDate, endDate]);

  useEffect(() => {
    fetchVisaApplications();
  }, [fetchVisaApplications]);

  const handleRequestSort = (property: keyof VisaApplication) => {
  ...
  const handleSelectAllClick = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = visaApplications.map((n) => n._id as string);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm(t("confirm-delete"))) {
      try {
        await VisaApplicationService.deleteVisaApplication(id);
        fetchVisaApplications();
      } catch (error) {
  ...
  const handleExport = () => {
    ...
    VisaApplicationService.exportVisaApplications(query);
  };
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        await VisaApplicationService.importVisaApplications(file);
      ...
      <Box className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <Box>
          ...
          <Breadcrumbs>
            ...
            <Typography color="text.primary">{t("Visa Applications")}</Typography>
          </Breadcrumbs>
        </Box>
        ...
        <Box className="flex items-center gap-2 flex-wrap">
          ...
          <Button
            ...
            onClick={() => navigate(`/${role}/visa-applications/create`)}
          >
            {t("New Application")}
          </Button>
        </Box>
        ...
        <Grid container spacing={2} alignItems="center">
          <Grid size={{ xs: 12, md: 3 }}>
            <TextField
              fullWidth
              size="small"
              placeholder={t("Search by passport number...")}
              value={search}
          ...
          <Grid size={{ xs: 12, md: 6 }} className="flex justify-end gap-2">
        ...
        <Table className="mt-4 w-full min-w-3xl">
          <TableHead className="border-b-1 border-gray-200">
            <TableRow>
              <TableCell sx={{ ...headCellSx, width: 48, pr: 0 }}>
                <Checkbox
                  color="primary"
                  indeterminate={selected.length > 0 && selected.length < visaApplications.length}
                  checked={visaApplications.length > 0 && selected.length === visaApplications.length}
                ...
                >
                  {headCell.id !== "actions" ? (
                    <TableSortLabel
                      ...
                      onClick={() => handleRequestSort(headCell.id as keyof VisaApplication)}
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={7} align="center" sx={{ ...cellSx, py: 8 }}>
                  <Typography variant="body1">{t("No visa applications found")}</Typography>
                </TableCell>
              </TableRow>
            ) : (
              visaApplications.map((visaApp, index) => {
                const id = visaApp._id as string;
                const isItemSelected = isSelected(id);
                const labelId = `visa-application-checkbox-${index}`;
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell component="th" id={labelId} scope="row" sx={{ ...cellSx, fontWeight: 500 }} onClick={() => navigate(`/${role}/visa-applications/view/${id}`)}>
                      {visaApp.passportNumber || "-"}
                    </TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/visa-applications/view/${id}`)}>{visaApp.visaType || "-"}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/visa-applications/view/${id}`)}>{formatDate(visaApp.applicationDate)}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/visa-applications/view/${id}`)}>{formatDate(visaApp.interviewDate)}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/visa-applications/view/${id}`)}>
                      <Typography color={getStatusColor(visaApp.status)}>{visaApp.status ? t(visaApp.status) : "-"}</Typography>
                    </TableCell>
                    <TableCell align="right" sx={cellSx}>
                      <Box className="flex justify-end gap-1">
                        <Tooltip title={t("View")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/visa-applications/view/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiEyeOpen size="medium" />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={t("Edit")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/visa-applications/edit/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiPen size="medium" />
                          </IconButton>
                        </Tooltip>
