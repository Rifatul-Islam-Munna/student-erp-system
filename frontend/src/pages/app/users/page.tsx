...
export default function UsersIndex() {
  ...
  const [orderBy, setOrderBy] = useState<keyof User>("createdAt" as any);
  ...
  const headCells: HeadCell[] = [
    ...
    { id: "role", label: t("Role"), numeric: false },
    { id: "accountStatus", label: t("Status"), numeric: false },
  ...
  const handleSelectAllClick = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = users.map((n) => n._id as string);
              ...
              users.map((user, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell component="th" id={labelId} scope="row" sx={{ ...cellSx, fontWeight: 500 }} onClick={() => navigate(`/${role}/users/view/${id}`)}>
                      {user.fullName}
                    </TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/users/view/${id}`)}>{user.email}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/users/view/${id}`)}>{user.phone}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/users/view/${id}`)}>{t(user.role)}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/users/view/${id}`)}>{t(user.accountStatus || "")}</TableCell>
                    <TableCell align="right" sx={cellSx}>
                      <Box className="flex justify-end gap-1">
                        <Tooltip title={t("View")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/users/view/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiEyeOpen size="medium" />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={t("Edit")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/users/edit/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiPen size="medium" />
                          </IconButton>
                        </Tooltip>
