...
export default function TasksIndex() {
  ...
  const [orderBy, setOrderBy] = useState<keyof Task>("createdAt" as any);
  ...
  const headCells: HeadCell[] = [
    ...
    { id: "assignedTo", label: t("Assigned To"), numeric: false },
    { id: "dueDate", label: t("Due Date"), numeric: false },
    { id: "priority", label: t("Priority"), numeric: false },
  ...
  }, [page, rowsPerPage, search, status, priority, startDate, endDate]);
  ...
  const handleSelectAllClick = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = tasks.map((n) => n._id as string);
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        await TaskService.importTasks(file);
    ...
    <Box className="p-4">
      <Box className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <Box>
          ...
          <Breadcrumbs>
            ...
            <Typography color="text.primary">{t("Tasks")}</Typography>
          </Breadcrumbs>
        </Box>
        ...
        <Box className="flex items-center gap-2 flex-wrap">
          ...
          <Button variant="surface" color="primary" startIcon={<NiPlus size="medium" />} onClick={() => navigate(`/${role}/tasks/create`)}>
            {t("New Task")}
          </Button>
        </Box>
      </Box>
      ...
      <Box className="bg-background-paper mb-4 p-4 rounded-xl shadow-sm">
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid size={{ xs: 12, md: 2 }}>
            <TextField fullWidth select size="small" placeholder={t("Priority")}
        ...
        <Table className="mt-4 w-full min-w-3xl">
          <TableHead className="border-b-1 border-gray-200">
            <TableRow>
              <TableCell sx={{ ...headCellSx, width: 48, pr: 0 }}>
                <Checkbox color="primary" indeterminate={selected.length > 0 && selected.length < tasks.length}
                  checked={tasks.length > 0 && selected.length === tasks.length} onChange={handleSelectAllClick}
                ...
                <TableCell key={headCell.id} align={headCell.numeric ? "right" : "left"}
                  ...
                  {headCell.id !== "actions" ? (
                    <TableSortLabel active={orderBy === headCell.id} direction={orderBy === headCell.id ? order : "asc"}
                      onClick={() => handleRequestSort(headCell.id as keyof Task)} IconComponent={NiArrowUp}>
                      {headCell.label}
                    </TableSortLabel>
                  ) : headCell.label}
                </TableCell>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={7} align="center" sx={{ ...cellSx, py: 8 }}><Typography variant="body1">{t("No tasks found")}</Typography></TableCell></TableRow>
            ) : (
              tasks.map((task, index) => {
                const id = task._id as string;
                const isItemSelected = isSelected(id);
                const labelId = `task-checkbox-${index}`;
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected} tabIndex={-1} key={id} selected={isItemSelected}
                    ...
                    <TableCell component="th" id={labelId} scope="row" sx={{ ...cellSx, fontWeight: 500 }} onClick={() => navigate(`/${role}/tasks/view/${id}`)}>{task.title}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/tasks/view/${id}`)}>{task.assignedTo || "-"}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/tasks/view/${id}`)}>{formatDate(task.dueDate)}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/tasks/view/${id}`)}>
                      <Chip label={t(task.priority || "low")} size="small" color={getPriorityColor(task.priority)} />
                    </TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/tasks/view/${id}`)}>
                      <Chip label={t(task.status || "pending")} size="small" color={getStatusColor(task.status)} />
                    </TableCell>
                    <TableCell align="right" sx={cellSx}>
                      <Box className="flex justify-end gap-1">
                        <Tooltip title={t("View")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/tasks/view/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiEyeOpen size="medium" />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={t("Edit")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/tasks/edit/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiPen size="medium" />
                          </IconButton>
                        </Tooltip>
