...
interface HeadCell {
  id: keyof SchoolSubmission | "actions";
  label: string;
  numeric: boolean;
}
...
export default function SchoolSubmissionsIndex() {
  ...
  const [orderBy, setOrderBy] = useState<keyof SchoolSubmission>("createdAt" as any);
  ...
  const headCells: HeadCell[] = [
    { id: "student", label: t("Student"), numeric: false },
    { id: "school", label: t("School"), numeric: false },
    { id: "applicationStatus", label: t("Application Status"), numeric: false },
    { id: "submittedDate", label: t("Submitted Date"), numeric: false },
    { id: "visaStatus", label: t("Visa Status"), numeric: false },
  ...
  const fetchSubmissions = useCallback(async () => {
    setLoading(true);
    try {
      const query: SchoolSubmissionQuery = {
        page: page + 1,
        limit: rowsPerPage,
        search: search,
      ...
      const response = await SchoolSubmissionService.getSchoolSubmissions(query);
      if (response.success) {
        setSubmissions(Array.isArray(response.data) ? response.data : []);
        setTotalCount(response.pagination?.total || 0);
      }
    } catch (error) {
      console.error("Failed to fetch school submissions", error);
    } finally {
      setLoading(false);
    }
  }, [page, rowsPerPage, search, applicationStatus, visaStatus, startDate, endDate]);

  useEffect(() => {
    fetchSubmissions();
  }, [fetchSubmissions]);

  const handleRequestSort = (property: keyof SchoolSubmission) => {
  ...
  const handleSelectAllClick = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = submissions.map((n) => n._id as string);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm(t("confirm-delete"))) {
      try {
        await SchoolSubmissionService.deleteSchoolSubmission(id);
        fetchSubmissions();
      } catch (error) {
  ...
  const handleExport = () => {
    ...
    SchoolSubmissionService.exportSchoolSubmissions(query);
  };
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        await SchoolSubmissionService.importSchoolSubmissions(file);
      ...
      <Box className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <Box>
          <Typography variant="h1" component="h1" className="mb-0">
            {t("School Submissions")}
          </Typography>
          <Breadcrumbs>
            <Link to={`/${role}/dashboards`}>{t("Home")}</Link>
            <Typography color="text.primary">{t("School Submissions")}</Typography>
          </Breadcrumbs>
        </Box>
        ...
        <Box className="flex items-center gap-2 flex-wrap">
          ...
          <Button
            ...
            onClick={() => navigate(`/${role}/school-submissions/create`)}
          >
            {t("New Submission")}
          </Button>
        </Box>
        ...
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid size={{ xs: 12, md: 3 }}>
            <FormControl fullWidth size="small">
              <InputLabel>{t("Application Status")}</InputLabel>
              <Select
          ...
          <Grid size={{ xs: 12, md: 3 }}>
            <FormControl fullWidth size="small">
              <InputLabel>{t("Visa Status")}</InputLabel>
              <Select
                value={visaStatus}
                label={t("Visa Status")}
          ...
          <Grid size={{ xs: 12, md: 3 }} className="flex justify-end gap-2">
        ...
        <Table className="mt-4 w-full min-w-3xl">
                ...
                >
                  {headCell.id !== "actions" ? (
                    <TableSortLabel
                      ...
                      onClick={() => handleRequestSort(headCell.id as keyof SchoolSubmission)}
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={8} align="center" sx={{ ...cellSx, py: 8 }}>
                  <Typography variant="body1">{t("No school submissions found")}</Typography>
                </TableCell>
              </TableRow>
            ) : (
              submissions.map((submission, index) => {
                const id = submission._id as string;
                const isItemSelected = isSelected(id);
                const labelId = `submission-checkbox-${index}`;
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell component="th" id={labelId} scope="row" sx={{ ...cellSx, fontWeight: 500 }} onClick={() => navigate(`/${role}/school-submissions/view/${id}`)}>
                      {submission.student || "-"}
                    </TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/school-submissions/view/${id}`)}>{submission.school || "-"}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/school-submissions/view/${id}`)}>{formatStatus(submission.applicationStatus)}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/school-submissions/view/${id}`)}>{formatDate(submission.submittedDate)}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/school-submissions/view/${id}`)}>{formatStatus(submission.visaStatus)}</TableCell>
                    <TableCell sx={cellSx} onClick={() => navigate(`/${role}/school-submissions/view/${id}`)}>{submission.intake || "-"}</TableCell>
                    <TableCell align="right" sx={cellSx}>
                      <Box className="flex justify-end gap-1">
                        <Tooltip title={t("View")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/school-submissions/view/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiEyeOpen size="medium" />
                          </IconButton>
                        </Tooltip>
                        <Tooltip title={t("Edit")}>
                          <IconButton size="small" onClick={() => navigate(`/${role}/school-submissions/edit/${id}`)}
                            sx={{ transition: "all 0.2s", "&:hover": { bgcolor: "primary.50", color: "primary.main" } }}>
                            <NiPen size="medium" />
                          </IconButton>
                        </Tooltip>
