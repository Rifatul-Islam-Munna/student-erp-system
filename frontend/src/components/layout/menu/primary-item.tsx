...
export const PrimaryItem = memo(function PrimaryItem({ item, onSelect, isActive, menuType, className }: Props) {
  ...
  const selected = useMemo(() => {
    ...
    if (item.href && isPathMatch(pathname, `${rolePrefix}${item.href}`)) return true;
    if (item.children) return item.children.some((child) => child.href && isPathMatch(pathname, `${rolePrefix}${child.href}`));

    return false;
  }, [item, pathname, rolePrefix]);
