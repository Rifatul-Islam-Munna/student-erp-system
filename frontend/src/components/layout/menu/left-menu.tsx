...
export default function LeftMenu() {
  ...
  const updateSelectedSecondaryItem = useCallback(() => {
    ...
    const findSelectedItem = (items: MenuItem[]): MenuItem | undefined => {
      for (const item of items) {
        if (item.href && isPathMatch(pathname, `${rolePrefix}${item.href}`)) {
          return item;
        }
        if (item.children) {
  ...
  }, [activeItem, pathname, rolePrefix, setMenuSelectedSecondaryItem]);

  useEffect(() => {
    let selectedMenu = leftMenuItems.find((item) => item.href && isPathMatch(pathname, `${rolePrefix}${item.href}`));
    if (!selectedMenu && leftMenuBottomItems) {
      selectedMenu = leftMenuBottomItems.find((item) => item.href && isPathMatch(pathname, `${rolePrefix}${item.href}`));
    }
  ...
  const handleSelectPrimaryItem = (item: MenuItem) => {
    ...
    if (item.content) {
      ...
      if (isPathMatch(pathname, `${rolePrefix}${item.href ?? ""}`)) {
